...
const IssueDetail = () => {
  ...
  const timelineSteps = [
    ...
    { label: "Ongoing", date: issue.status !== "Pending" ? issue.updatedAt : undefined, done: ["Ongoing", "Resolved", "Escalated"].includes(issue.status) },
    { label: "Resolved", date: issue.resolvedAt, done: issue.status === "Resolved" },
  ];
      ...
      <div className="grid gap-8 lg:grid-cols-5">
        <div className="lg:col-span-3 space-y-4">
          ...
          {issue.proofImage && issue.status === "Resolved" && (
            <div className="rounded-xl border bg-accent/30 p-4">
              ...
              <img src={issue.proofImage} alt="Resolution proof" className="max-h-60 w-full rounded-lg object-cover" />
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="mb-3 flex items-start gap-3 flex-wrap">
              ...
              {isEscalated && (
                <span className="inline-flex items-center gap-1 rounded-full border border-destructive/20 bg-destructive/5 px-3 py-0.5 text-label font-medium text-destructive">
                  <AlertTriangle className="h-3 w-3" /> Escalated to Municipal Corporation
                </span>
              )}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="rounded-md bg-secondary px-2.5 py-1 text-label text-secondary-foreground">{issue.category}</span>
            <span className="rounded-md bg-secondary px-2.5 py-1 text-label text-secondary-foreground">{issue.department}</span>
            {issue.reporters > 1 && (
              ...
              {isEscalated && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {issue.escalatedAt && <p className="text-label text-muted-foreground">{new Date(issue.escalatedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>}
                  </div>
                </div>
              )}
            </div>
