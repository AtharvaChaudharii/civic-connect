...
const categories: IssueCategory[] = ["Garbage", "Pothole", "Water Overflow", "Street Light", "Drainage", "Footpath", "Other"];
const departmentMap: Record<IssueCategory, string> = {
  Garbage: "Sanitation", Pothole: "Roads & Infrastructure", "Water Overflow": "Water Supply",
  "Street Light": "Electrical", Drainage: "Drainage", Footpath: "Roads & Infrastructure", Other: "General",
};
const urgencyLevels = ["Normal (Can wait a few days)", "Urgent (Needs attention soon)", "Critical (Immediate danger)"];
        ...
        <div className="mx-auto max-w-xl text-center">
          ...
          <div className="mb-8 rounded-xl border bg-card p-6 text-left shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-0.5 text-label font-medium text-amber-800">Pending Review</span>
            </div>
            <div className="rounded-lg bg-muted/50 p-4">
              <h4 className="text-body font-semibold text-foreground">{description ? description.slice(0, 40) : "New Issue Report"}</h4>
              <p className="mt-1 flex items-center gap-1 text-caption text-muted-foreground"><MapPin className="h-3.5 w-3.5" />{location || "Location not specified"}</p>
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div><p className="text-label uppercase tracking-wider text-muted-foreground">Category</p><p className="text-caption font-medium text-foreground">{category || "General"}</p></div>
                <div><p className="text-label uppercase tracking-wider text-muted-foreground">Report ID</p><p className="text-caption font-medium text-foreground">#PUN-2023-{Math.floor(Math.random() * 9000 + 1000)}</p></div>
                <div><p className="text-label uppercase tracking-wider text-muted-foreground">Date</p><p className="text-caption font-medium text-foreground">{new Date().toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p></div>
                <div><p className="text-label uppercase tracking-wider text-muted-foreground">Est. Resolution</p><p className="text-caption font-medium text-primary">7 Days</p></div>
              </div>
            </div>
          </div>
          ...
          <p className="mt-4 text-caption text-muted-foreground">You will receive email updates as the status of your report changes.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-6 rounded-xl border bg-card p-6 shadow-sm">
          ...
          {imagePreview ? (
            <div className="relative mb-3">
              ...
              <button onClick={() => setImagePreview(null)} className="absolute top-2 right-2 rounded-lg bg-card/90 px-3 py-1 text-label font-medium text-foreground">Remove</button>
            </div>
          ) : (
          ...
          <div className="mt-3 space-y-2">
            ...
            <Input placeholder="e.g. A large pothole in the middle of the crosswalk" value={imageDesc} onChange={(e) => setImageDesc(e.target.value)} className="h-10" />
          </div>
        </div>

        {/* Section 2: Location with Map */}
        <div className="mb-6 rounded-xl border bg-card p-6 shadow-sm">
          <div className="mb-4 flex items-center gap-3">
          ...
          <div className="mb-3 overflow-hidden rounded-xl border">
            <IssueMap
              issues={[]}
              singlePin={[pinLat, pinLng]}
              draggablePin
              onPinMove={(lat, lng) => { setPinLat(lat); setPinLng(lng); }}
              height="h-52"
            />
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={urgency} onValueChange={setUrgency}>
                ...
                <SelectContent>{urgencyLevels.map((u) => (<SelectItem key={u} value={u}>{u}</SelectItem>))}</SelectContent>
              </Select>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <p className="text-caption text-muted-foreground">
            By submitting, you agree to our <a href="#" className="text-primary hover:underline">Terms of Service</a> and <a href="#" className="text-primary hover:underline">Privacy Policy</a>.
          </p>
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => navigate("/dashboard")}>Cancel</Button>
            <Button onClick={handleSubmit} disabled={!canSubmit} className="gap-2"><Send className="h-4 w-4" /> Submit Report</Button>
          </div>
        </div>
      </div>
    </div>
