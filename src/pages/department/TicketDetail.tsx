    ...
    <div className="civic-container civic-section">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" className="gap-1.5 border-destructive/30 text-destructive hover:bg-destructive/5" onClick={handleEscalate}>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="space-y-1 text-caption">
                <div className="flex justify-between"><span className="text-muted-foreground">Category:</span><span className="font-medium text-foreground">{issue.category}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Priority:</span><span className="font-medium text-destructive">! High</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Assigned To:</span><span className="font-medium text-foreground">Team Alpha</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-card p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="rounded-full bg-accent px-2.5 py-0.5 text-label font-medium text-primary">{issue.reporters} Reporters</span>
            </div>
            <p className="mb-3 text-caption text-muted-foreground">All updates here will notify linked reporters automatically.</p>
              ...
              {["Rahul K.", "Sneha M."].map((name, idx) => (
                <div key={name} className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-label font-medium text-primary">{name.slice(0, 2).toUpperCase()}</div>
                    <div>
                  ...
                  <span className="text-label text-muted-foreground">#{8955 - idx * 23}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-card p-5">
            ...
            {proofPreview ? (
              <div className="relative mb-3">
                ...
                <button onClick={() => setProofPreview(null)} className="absolute top-2 right-2 rounded bg-card/90 px-2 py-1 text-label text-foreground">Remove</button>
              </div>
            ) : (
            ...
            <div className="mb-4 space-y-2">
              ...
              <Textarea placeholder="Describe the work done..." value={resolutionNote} onChange={(e) => setResolutionNote(e.target.value)} rows={3} />
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="outline">Cancel</Button>
              <Button disabled={!proofPreview} className="gap-2" onClick={handleResolve}>
              ...
              {timeline.map((item, idx) => (
                <div key={idx} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={cn("flex h-3 w-3 rounded-full", item.active ? "bg-primary" : "bg-muted-foreground/30")} />
                  ...
                  <div className="pb-4">
                    ...
                    {item.description && <p className="mt-1 text-caption text-muted-foreground">{item.description}</p>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3 mb-4 max-h-80 overflow-y-auto">
              {discussionMessages.map((msg, idx) => (
                <div key={idx} className={cn("flex gap-2", msg.isOwn && "flex-row-reverse")}>
                  ...
                  <div className={cn("max-w-[80%] rounded-xl px-3 py-2", msg.isOwn ? "bg-primary text-primary-foreground" : "bg-muted")}>
                  ...
                  {msg.isOwn && (
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary text-label font-semibold text-primary-foreground">You</div>
                  )}
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              <Textarea placeholder="Type a comment..." value={comment} onChange={(e) => setComment(e.target.value)} rows={1} className="flex-1 min-h-[40px]" />
              <Button disabled={!comment.trim()} size="icon" className="shrink-0 h-10 w-10" onClick={handleSendComment}>
